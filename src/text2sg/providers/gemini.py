"""Gemini provider for scene graph generation."""
...
class GeminiProvider(BaseGenerator):
    """Gemini provider for scene graph generation using Google Gemini 2.0 Flash API."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.api_url = self.config.get('api_url', 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent')
                ...
                "output": [
                    {
                        ...
                        "edges": [["cat", "on", "windowsill"], ["cat", "watches", "rain"]]
                    },
                    {
                        "time": "T2",
                        "nodes": [
                            ...
                            {"id": "windowsill", "attributes": []},
                            {"id": "milk_bowl", "attributes": []}
                        ],
                        "edges": [["cat", "jumps_from", "windowsill"], ["cat", "walks_toward", "milk_bowl"]]
                    }
                ]
            }
        ]
...
"""
    ...
    def generate(self, text: str, **kwargs) -> GenerationResult:
        """Generate scene graph using Gemini API.
        ...
        try:
            ...
            self.logger.info(f"Generating scene graph with Gemini model")
            ...
            # Parse JSON response
            scene_data = self._extract_and_parse_json(generated_text)
            if not scene_data:
                return GenerationResult(
                    success=False,
                    scenegraph=None,
    ...
    def _extract_and_parse_json(self, text: str) -> Optional[List[Dict[str, Any]]]:
        ...
        # Try to extract JSON from text using regex patterns
        json_patterns = [
            r'```json\s*(\[.*?\])\s*```',
            r'```\s*(\[.*?\])\s*```',
            r'(\[\s*{.*?}\s*\])',
    ...
    def _validate_scene_graph_item(self, item: Dict[str, Any]) -> bool:
        """Validate a scene graph item structure.
        
        Args:
            item: Scene graph item to validate
        ...
        if 'time' not in item or 'nodes' not in item or 'edges' not in item:
            return False
    ...
    def generate_with_colors(self, text: str, **kwargs) -> str:
        """Enrich text description with color information using Gemini API.
...
Output (enriched text only, no additional text):""" 
            ...
            enriched_text = result['candidates'][0]['content']['parts'][0]['text'].strip()
